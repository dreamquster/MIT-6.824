...
func doMap(
	...
	sliceKeyValues := mapF(inFile, string(contentBytes))
	var reduceEnc = make([]*json.Encoder, nReduce);
	for r := 0; r < nReduce; r++ {
		...
		reduceEnc[r] = json.NewEncoder(file);
	}



	for _, kv := range sliceKeyValues {
		r := ihash(kv.Key) % nReduce;
		err := reduceEnc[r].Encode(&kv)
		if (nil != err) {
			panic(err)
		}
	}
